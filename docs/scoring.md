...
- **ルール（4割）**: 決定的でブレない。キーワードでしか判断できないのが弱点
- **Gemini（6割）**: 文脈を読める（「7割」が進捗なのか別の意味なのか等）。ただし毎回少し揺れる
- 「質問に答えていない」などルールが確実に検出した注意点は、Gemini の結果に関係なく必ず表示します

## 既知の限界

- ルールは日本語のキーワード一致なので、言い回しを変えると取りこぼす（例: 「片付きました」は完了と判定されない）。Gemini 併用時は文脈で補正されます
- 受信力は「質問の単語が答えに含まれるか」で見ているため、別の言葉で答えると答え漏れ扱いになることがあります
